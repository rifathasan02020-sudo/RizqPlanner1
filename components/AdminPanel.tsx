    ...
    <div className="space-y-8 animate-fade-in-up pb-20">
      <div className="flex items-center gap-3 border-b border-white/10 pb-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">ইউজার ও ডাটাবেস ওভারভিউ</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-slate-900/50 border-white/5">
          <div className="flex items-center gap-3 mb-2 text-cyan-400"><Users size={24} /><h3 className="font-bold">মোট ইউজার</h3></div>
          <p className="text-3xl font-bold text-white">{profiles.length}</p>
        </Card>
        <Card className="bg-slate-900/50 border-white/5">
          <div className="flex items-center gap-3 mb-2 text-emerald-400"><Database size={24} /><h3 className="font-bold">লেনদেন এন্ট্রি</h3></div>
          <p className="text-3xl font-bold text-white">{totalTxns}</p>
        </Card>
        <Card className="bg-slate-900/50 border-white/5">
          <div className="flex items-center gap-3 mb-2 text-purple-400"><Database size={24} /><h3 className="font-bold">মোট সঞ্চয়</h3></div>
          <p className="text-3xl font-bold text-white">৳ {totalSavings.toLocaleString()}</p>
        </Card>
      </div>

      <Card className="overflow-hidden">
        <h3 className="text-lg font-bold text-white mb-4 px-2">ইউজার তালিকা</h3>
              ...
              {profiles.map((profile) => (
                <tr key={profile.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="p-3 flex items-center gap-2">
                     <img src={profile.avatar_url || getAvatar()} className="w-6 h-6 rounded-full" />
                     {profile.name}
                  </td>
